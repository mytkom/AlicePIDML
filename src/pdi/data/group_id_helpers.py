...
def filter_strings_by_binary(
    integer_filter: int, array: NDArray, persist_key: Literal["0"] | Literal["1"] = "1"
) -> NDArray:
...
def group_id_to_detectors_available(group_id: GroupID) -> list[Detector]:
    missing_columns = filter_strings_by_binary(
        int(group_id), np.array(COLUMNS_FOR_TRAINING), "0"
    )
